...
############################## Part 1: your code begins here ###
	...
	#Total number of indexes (Stored in $t0)
	mult $a2, $a2
	mflo $t0 #$a2 squared
	
	li $t1, 0 #Index counter (Increments by 1)
	#Loop through the 2d array
	thresholdLoop:
		
		#Check if gone through entire array
		beq $t1, $t0, thresholdLoopDone
		
		#Load value at address
		add $t3, $a0, $t1
		lbu $t4, 0($t3)
		
		#Check if value >= or < threshold
		bge $t4, $a3, thresholdLoopVGTE
		
		thresholdLoopVLT:
			add $t3, $a1, $t1 #output buffer address
			sb $t6, 0($t3)
			j thresholdLoopNext
...
############################### Part 2: your code begins here ##
	...
	li $t0, 0 #Row index of OUTPUT ARRAY
	transformRowLoop:
		beq $t0, $a3, transformRowLoopDone
	
		li $t1, 0 #Column index of OUTPUT ARRAY
		transformColLoop:
			beq $t1, $a3, transformColLoopDone
			
			#Use transform matrix to calculate ORIGIN INDEXES
			move $t3, $t1 #Transformed COL index
			move $t2, $t0 #Transformed ROW index
				#Transform x index 
				lw $t4, 0($a2)
				lw $t5, 4($a2)
				lw $t6, 8($a2)
				...
				mult $t5, $t2 #M01 * Y
				mflo $t9
				add $t9, $t8, $t9 #(M00 * X) + (M01 * Y)
				add $t9, $t9, $t6 #(M00 * X) + (M01 * Y) + M02
				...
				mult $t5, $t2 #M01 * Y
				mflo $t5
				add $t4, $t4, $t5 #(M00 * X) + (M01 * Y)
				add $t8, $t4, $t6 #(M00 * X) + (M01 * Y) + M02 
				
				#Move transformed indexes to $t2, $t3 registers
				move $t3, $t9
				move $t2, $t8
				
				#Check indexes
					#Check COL >= pixel
					bge $t3, $a3, transformColLoopCheckFail
					 
					#Check COL < 0
					blt $t3, 0, transformColLoopCheckFail
					
					#Check ROW >= pixel
					bge $t2, $a3, transformColLoopCheckFail
					 
					#Check ROW < 0
					blt $t2, 0, transformColLoopCheckFail
			
			#Find offset (after using transform matrix) of INPUT ARRAY
			mult $t2, $a3
			mflo $t4
			add $t4, $t4, $t3
			
			#Find element address (after using transform matrix)
			add $t4, $t4, $a0
			
			#Move the pixel value to the transformed index
				#Calculate original address of pixel
					#Calculate offset
					mult $t0, $a3
					mflo $t5
					add $t5, $t5, $t1
				...
				#Store the pixel value to OUTPUT index
				sb $t4, 0($t5)
			
			j transformColLoopCheckSuccess
			
			#Index out of bounds
			transformColLoopCheckFail:
			mult $t0, $a3
			mflo $t5
			add $t5, $t5, $t1
			add $t5, $t5, $a1
			sb $0, 0($t5) #Set pixel value to 0
			
			transformColLoopCheckSuccess:
			addi $t1, $t1, 1
			j transformColLoop
			
		transformColLoopDone:
		addi $t0, $t0, 1
		j transformRowLoop
		
	transformRowLoopDone:
